...
var Potrace = (function () {
    ...
    var imgCanvas = new OffscreenCanvas(1, 1),
        bm = null,
        pathlist = [],
        callback,
        info = {
    ...
    function pathToString(curve, size) {
        function bezier(i) {
            var b =
                'C ' +
    ...
    function getSVGPaths(size) {
        var paths = new Array(pathlist.length);
        var len = pathlist.length,
            ...
            fillrule;
        for (i = 0; i < len; i++) {
            paths[i] = pathToString(pathlist[i].curve, 1);
        }
    ...
    function getSVG(size, opt_type) {
        ...
        var style;
        var close =
            '" stroke="' + strokec + '" fill="' + fillc + '"' + fillrule + '/>';

        svg += open + paths.join(close + open) + close;
